...
function ModoAWizard({ token, socket, onClose, onImported, sourceType = "EXCEL", externalData }) {
  ...
  const confirmImport = async () => {
    ...
    // Conectores vivos: no hay archivo que subir — /sync solo ENCOLA el job
    // real (BullMQ) y regresa de inmediato; el resultado llega por
    // Socket.io (ver useEffect de "sync:complete"/"sync:error" arriba).
    if (isLiveConnector) {
      setProgress({ processed: 0, total: preview?.totalRows || 0 });
      try {
        const syncFn = LIVE_CONNECTOR_SYNC[sourceType];
        if (syncFn) await syncFn(token);
      } catch (e) {
  ...
  const liveConnectorLabel = LIVE_CONNECTOR_LABELS[sourceType];
  const wizardTitle = liveConnectorLabel ? `Sincronizar ${liveConnectorLabel}` : "Exportación manual";
  const wizardSubtitle = liveConnectorLabel ? `${liveConnectorLabel} · Mapeo e importación` : "MODO A · Configuración";
