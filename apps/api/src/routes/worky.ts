...
import {
  WorkyCredentials, WORKY_FIELD_MAP,
  getWorkyCredentials, saveWorkyCredentials, deleteWorkyCredentials,
  fetchAllWorkyCollaborators, mapWorkyRecordToEmployee, testWorkyConnection,
} from '../connectors/worky/workyConnector'
import { upsertEmployee } from './connectors'
import { queueWorkySync } from '../jobs/workySyncQueue'

const router = Router()

const SOURCE = 'WORKY'

interface WorkySyncRowError { employeeId?: string; message: string }
interface WorkySyncResult   { processed: number; inserted: number; updated: number; errors: WorkySyncRowError[] }

// ── connected_sources: solo metadata (sin archivo real que guardar) ──

async function upsertWorkyConnectedSource(tenantDb: any, tenantId: string): Promise<void> {
  const fileName = 'Worky (API)'
  const fileContent = JSON.stringify({ connectedAt: new Date().toISOString() })
  const checksum = crypto.createHash('md5').update(`${tenantId}:worky`).digest('hex')
...
async function markWorkySourceSynced(tenantDb: any, tenantId: string, status: 'CONNECTED' | 'ERROR', lastError: string | null): Promise<void> {
  await tenantDb.$executeRaw`
...
// ── POST /api/connectors/worky/connect ───────────────────────
...
router.post('/connect', requireHR, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const creds: WorkyCredentials = { token: apiKey }

    let employeeCount: number
    let sample: ReturnType<typeof mapWorkyRecordToEmployee>[]
    try {
      ({ employeeCount, sample } = await testWorkyConnection(creds))
    } catch (err: any) {
      throw new AppError(400, `Credenciales inválidas o sin permisos: ${err.message}`)
    }

    await saveWorkyCredentials(tenantId, creds)
    await upsertWorkyConnectedSource(req.tenantDb, tenantId)
...
// ── POST /api/connectors/worky/sync ──────────────────────────
...
router.post('/sync', requireHR, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const jobId = await queueWorkySync(tenantId)
...
// ── GET /api/connectors/worky/status ─────────────────────────
...
// ── Orquestación del sync real (llamada por el worker de workySyncQueue) ──

export async function runWorkySync(tenantId: string, tenantDb: any, io: any): Promise<WorkySyncResult> {
  const errors: WorkySyncRowError[] = []
  let inserted = 0
  let updated = 0

  const creds = await getWorkyCredentials(tenantId)
  if (!creds) {
    await markWorkySourceSynced(tenantDb, tenantId, 'ERROR', 'Worky no está conectado')
    return { processed: 0, inserted: 0, updated: 0, errors: [{ message: 'Worky no está conectado' }] }
  }
  ...
  try {
    ...
    for (const raw of records) {
      const row = mapWorkyRecordToEmployee(raw)
      const workyId = String(raw['collaborator_id'] ?? '')
      try {
        const { outcome } = await upsertEmployee(tenantDb, tenantId, row, SOURCE)
        if (outcome === 'inserted') inserted++
        else updated++
      } catch (err: any) {
        errors.push({ employeeId: workyId, message: err.message })
      }
    }

    const allFailed = records.length > 0 && errors.length === records.length
    await markWorkySourceSynced(
      tenantDb, tenantId,
      allFailed ? 'ERROR' : 'CONNECTED',
      errors.length > 0 ? `${errors.length} registro(s) con error` : null,
    )
  } catch (err: any) {
    await markWorkySourceSynced(tenantDb, tenantId, 'ERROR', err.message)
    errors.push({ message: err.message })
  }

  const result: WorkySyncResult = { processed: inserted + updated, inserted, updated, errors }
