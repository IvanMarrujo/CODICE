// ============================================================
// CÓDICE · Buk sync queue (BullMQ, in-process)
...
export const bukSyncQueue = new Queue(QUEUE_NAME, { connection })

interface BukSyncJobData {
  tenantId: string
}

type SyncFn = (tenantId: string, tenantDb: any, io: any) => Promise<unknown>

let worker: Worker<BukSyncJobData> | null = null

/** Arranca el worker una sola vez, desde index.ts — mismo motivo que startAutoSyncWorker (io para Socket.io). */
export function startBukSyncWorker(io: any, runSync: SyncFn): Worker<BukSyncJobData> {
  if (worker) return worker

  worker = new Worker<BukSyncJobData>(
    QUEUE_NAME,
    async (job: Job<BukSyncJobData>) => {
  ...
  worker.on('failed', (job, err) => {
    console.error(`❌  job "buk-sync" falló (tenant ${job?.data.tenantId}):`, err.message)
...
export async function queueBukSync(tenantId: string): Promise<string> {
  const job = await bukSyncQueue.add('buk-sync', { tenantId })
  return job.id!
}
